...
function loadDB(){
  ...
  if(v2){try{const d=JSON.parse(v2);d.catalog=d.catalog||clone(DEFAULT_CATALOG);d.settings=d.settings||{minReviewMs:500,studentSiteOpen:true};d.students=d.students||{};Object.values(d.students).forEach(normalizeStudent);return d;}catch(e){console.warn(e);}}
  const old=localStorage.getItem(OLD_DB_KEY);
  if(old){try{const o=JSON.parse(old);o.catalog=clone(DEFAULT_CATALOG);o.settings={minReviewMs:500,studentSiteOpen:true};Object.values(o.students||{}).forEach(normalizeStudent);saveRaw(o);return o;}catch(e){console.warn(e);}}
  const students={};DEFAULT_STUDENTS.forEach(n=>students[n]=blankStudent());return {students,currentStudent:DEFAULT_STUDENTS[0],catalog:clone(DEFAULT_CATALOG),settings:{minReviewMs:500,studentSiteOpen:true}};
}
...
function visibleCatalog(track){return (db.catalog[track]||[]).filter(x=>!x.hidden);}
...
function learnedCards(s=student(),track=null,includeHidden=true){return Object.values(s.cards).filter(c=>c.learnedAt&&(!track||c.itemType===track)&&(includeHidden||!getItem(c.itemKey)?.hidden)).sort((a,b)=>a.learnedAt-b.learnedAt);}
...
function renderArtChoices(item){const emojis=document.getElementById('emojiChoices'),ills=document.getElementById('illustrationChoices');emojis.innerHTML='';ills.innerHTML='';(item.art||[]).forEach(value=>{const b=document.createElement('button');b.type='button';b.className='art-choice';b.textContent=value;b.onclick=()=>addArtToEditor({kind:'emoji',value,label:value});emojis.appendChild(b);});const choices=item.id?ILLUSTRATIONS[item.id]||[]:[];if(!choices.length){ills.innerHTML='<span class="mini-note">No illustration for this item yet.</span>';return;}choices.forEach(a=>{const b=document.createElement('button');b.type='button';b.className='art-choice';b.innerHTML=`<img src="${a.src}" alt="${escapeHTML(a.label)}">`;b.onclick=()=>addArtToEditor({kind:'illustration',value:a.src,label:a.label});ills.appendChild(b);});}
...
function bindReview(){
  document.querySelectorAll('[data-review-perek]').forEach(btn=>btn.onclick=()=>{reviewPerekFilter=btn.dataset.reviewPerek;document.querySelectorAll('[data-review-perek]').forEach(x=>x.classList.toggle('active',x===btn));renderReviewPicker();});
