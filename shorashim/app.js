...
function reviewSortedCards(){let arr=eligibleLearnedCards().slice(),mode=document.getElementById('reviewSort')?.value||'learned';if(mode==='alpha')arr.sort((a,b)=>(getItem(a.itemKey)?.front||'').localeCompare(getItem(b.itemKey)?.front||'','he'));else if(mode==='manual')arr.sort((a,b)=>(a.reviewOrder??a.learnedAt)-(b.reviewOrder??b.learnedAt));else arr.sort((a,b)=>a.learnedAt-b.learnedAt);return arr;}
function renderReviewPicker(){const box=document.getElementById('reviewPicker'),arr=reviewSortedCards(),manual=document.getElementById('reviewSort').value==='manual';box.innerHTML='';arr.forEach(c=>{const item=getItem(c.itemKey),row=document.createElement('div');row.className='review-pick-row'+(manual?' manual-draggable':'');row.dataset.key=c.itemKey;row.draggable=manual;row.innerHTML=`<label><input class="review-pick-check" type="checkbox" data-key="${escapeHTML(c.itemKey)}" checked> <b class="hebrew">${escapeHTML(item.front)}</b><span>${escapeHTML(item.english)}</span>${manual?'<span class="review-drag-hint">↕ Drag to reorder</span>':''}</label>`;box.appendChild(row);});box.querySelectorAll('.review-pick-check').forEach(x=>x.onchange=updateReviewSelectionCount);if(manual)bindReviewDragDrop(box);updateReviewSelectionCount();}
